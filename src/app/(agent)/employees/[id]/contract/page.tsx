...
export default function WorkPermitPage({
  ...
  const { data: employmentContractHistory, isLoading: isLoadingContracts } =
    useQuery({
      ...getEmploymentContractHistoryQueryOption({
        passportNo: id,
  ...
  return (
    <div className="flex flex-col gap-[51px] w-full px-[20px] md:px-[36px] py-[8px] md:py-[20px]">
      {/* HeaderSection */}
      <HeaderSection topic="สัญญาจ้าง 3 ภาษา" hasBackButton={true} />

      {/* WorkPermitFormSection */}
      <EmploymentContactForm
        employeeId={id}
        employerId={employeeData?.currentEmployer.employerId ?? ""}
        employmentContractHistory={employmentContractHistory ?? []}
      />
